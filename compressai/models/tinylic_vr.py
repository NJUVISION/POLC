...
class TinyLICVR(nn.Module):
    r"""Variable-rate version of TinyLIC
    ...
    def g_s(self, x, quality):
        x, decisions_g_s0 = self.g_s0(x, quality=quality)
        x = self.g_s1(x)
        x, decisions_g_s2 = self.g_s2(x, quality=quality)
        x = self.g_s3(x)
        x, decisions_g_s4 = self.g_s4(x, quality=quality)
        x = self.g_s5(x)
        x, decisions_g_s6 = self.g_s6(x, quality=quality)
        x = self.g_s7(x)
    ...
    def forward_features(self, x, quality=1):
        ...
        x_hat, dec_decisions = self.g_s(y_hat * torch.abs(self.InverseGain[quality-1]).unsqueeze(0).unsqueeze(2).unsqueeze(3), quality)
    ...
    def forward(self, x, quality=1):
        x = self.forward_features(x, quality)
        return x
    ...
    def decompress(self, strings, shape, quality=1, q_task=0, task_idx=0):
        ...
        x_hat, _ = self.g_s(y_hat, quality)
